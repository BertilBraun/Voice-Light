...
run_compile_stage() {
  ...
  if [[ ! -f "$stage_output/compiled/synthetic-corpus.json" ]]; then
    "$qwen_environment/bin/python" -m app.local.synthetic_generation.cli \
      ...
      --split-seed voice-light-synthetic-conversations-20h-v2 \
      --crop-variants 1 \
...
run_prompt_stage "$preflight" voice_light_conversation_preflight_v2 --count 10
...
run_prompt_stage \
  "$corpus" \
  voice_light_synthetic_conversations_20h_v2 \
  --count 2000 \
