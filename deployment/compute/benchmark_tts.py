...
from app.compute.voice.kyutai_tts import KyutaiSpeechSynthesizer, KyutaiTtsConfiguration
...
def main(arguments: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument("--codebook-count", type=int, default=32)
    options = parser.parse_args(arguments)
    if options.runs < 1:
        raise ValueError("--runs must be at least 1.")
    asyncio.run(run_benchmark(options.text, options.runs, options.codebook_count))


async def run_benchmark(text: str, runs: int, codebook_count: int) -> None:
    load_started = time.perf_counter()
    synthesizer = await asyncio.to_thread(
        KyutaiSpeechSynthesizer,
        KyutaiTtsConfiguration(codebook_count=codebook_count),
    )
    ...
    try:
        for run_index in range(runs):
            ...
            print(
                f"codebook_count={codebook_count} run={run_index + 1} "
