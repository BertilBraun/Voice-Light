...
@pytest.mark.parametrize(
    ...
    (
        ...
        '{"name":"search","arguments":{"query":"London"},"extra":true}',
    ),
)
...
def test_duplicate_call_is_rejected_after_first_call_is_buffered() -> None:
    envelope = (
        f'{TOOL_CALL_START}{{"name":"search","arguments":{{"query":"London"}}}}{TOOL_CALL_END}'
    )
...
def test_multiple_calls_are_rejected_after_first_call_is_buffered() -> None:
    first = f'{TOOL_CALL_START}{{"name":"search","arguments":{{"query":"London"}}}}{TOOL_CALL_END}'
    second = f'{TOOL_CALL_START}{{"name":"search","arguments":{{"query":"Berlin"}}}}{TOOL_CALL_END}'
...
def assert_search_events(events: tuple[HermesParserEvent, ...]) -> None:
    ...
    assert len(calls) == 1
    assert calls[0].request.name == "search"
    assert calls[0].request.arguments_json == '{"query":"London"}'
    assert failures == []
    assert "<tool_call>" not in spoken_text(events)
    assert "query" not in spoken_text(events)
