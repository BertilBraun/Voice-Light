...
def test_tool_prompt_requires_spoken_bridge_before_call() -> None:
    ...
    assert "must call search for niche or uncertain factual claims" in LANGUAGE_MODEL_SYSTEM_PROMPT
    assert "supplies contrary firsthand evidence" in LANGUAGE_MODEL_SYSTEM_PROMPT
    assert "Do not answer these requests from memory" in LANGUAGE_MODEL_SYSTEM_PROMPT
